...
class PagamentoController extends Controller
{
    ...
    public function index()
    {
        ...
        $dados["lista"]         = Pagamento::filtro($filtro);
        $dados["filtro"]        = $filtro;
        $dados["mes"]           = 13;
        $dados["formas"]        = FormaPagto::get();
        return view("Financeiro.Pagamento.Index", $dados);
    }
    ...
    public function pormes()
    {
        $mes            = $_GET["mes"];
        $ano            = $_GET["ano"];
        $dados["lista"] = Pagamento::whereMonth("data_pagamento", $mes)->whereYear("data_pagamento", $ano)->get();
        $dados["mes"]   = $mes;
        $dados["formas"]= FormaPagto::get();
        return view("Financeiro.Pagamento.Index", $dados);
    }
    ...
    {
        $dados["pagamento"]    = Pagamento::find($id);
        $dados["formaPagto"]    = FormaPagto::all();
        $dados["fornecedores"]  = Fornecedor::get();

        $dados["pagamentos"]    = array();
        return view("Financeiro.Pagamento.ConfirmarPagamento", $dados);
    }
    ...
    public function create()
    {
        ...
        $dados["pagamentos"] = array();
        return view("Financeiro.Pagamento.Create", $dados);
    }
    ...
    public function pagar(Request $request)
    {
        ...
        if($pag){
            Pagamento::where("id", $req["conta_pagar_id"])->update(["pagamento_id"=>$pag->id, "status_id"=>config("constantes.status.PAGO")]);
        }
    ...
    public function show($id)
    {
        ...
        $dados["pagamentos"] = array();
        return view("Financeiro.Pagamento.Detalhe", $dados);
    }
