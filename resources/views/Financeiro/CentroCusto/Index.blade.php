            ...
            <div class="caixa">
                ...
                <div class="px-2 py-2 w-100 d-grid">
                    @if (isset($centrocusto))
                        <form action="{{ route('centrocusto.update', $centrocusto->id) }}" method="POST">
                            @method('put')
                        @else
                            <form action="{{ route('centrocusto.store') }}" method="POST">
                    @endif
                    @csrf
                    <div class="{{ isset($centrocusto->nome) ? 'bg-edit' : 'caixafield' }}  p-2 radius-4 border">
                        <div class="   p-2 pt-0 radius-4">
                            <div class="rows center-middle">
                                <div class="col-9">
                                    <label class="text-label d-block text-branco">Nome </label>
                                    <input type="text" name="nome" value="{{ $centrocusto->nome ?? old('nome') }}"
                                        class="form-campo">
                                </div>
                        ...
                        <tbody>
                            @foreach ($lista as $l)
                                <tr>
                                    ...
                                    <td align="center">
                                        <a href="{{ route('centrocusto.edit', $l->id) }}"
                                        ...
                                        <a href="javascript:;"
                                            ...
                                            <form action="{{ route('centrocusto.destroy', $l->id) }}" method="POST"
                                                id="apagar{{ $l->id }}">
                                                @method('DELETE')
                                                @csrf
                                            </form>
